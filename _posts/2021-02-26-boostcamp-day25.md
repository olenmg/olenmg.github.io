...
따라서 새로운 정점이 들어오더라도 무언가 새로 학습시키거나 할 필요 없이 기존에 사용하면 집계 함수를 이용하여 해당 정점의 임베딩을 찾을 수 있다.  
...
그림에는 그려져있지않지만 사실 target 정점의 임베딩을 구하기 위해 **이전 층에서 target 정점의 정보 벡터를 input으로 함께 넣어주어야**한다.  
  
최종적으로 $K$번째 layer의 output $z\_v$가 우리가 원하는 정점의 임베딩 벡터가 된다.   
  
그래프 신경망에서의 손실함수는 어떻게 설게할 수 있을까? 
단순히 설계하면 MSE 등의 손실함수를 사용할 수 있을 것 같은데, 시렞로는 **downstream task에 기반하여 end-to-end 학습을 할 수 있기 때문에**
...
$$
\mathcal{L} = \sum _{v \in V} y_v \log (\sigma(\mathrm{z}_v ^{\intercal} \theta)) + (1 - y_v) \log (1 - \sigma(\mathrm{z}_v ^{\intercal} \theta) )
$$
...
이러한 학습을 거치면 학습 데이터로 학습된 신경망을 적용하여 학습에 사용되지 않은 정점의 임베딩을 얻을 수 있다. 
뿐만 아니라 학습 이후에 추가된 정점의 임베딩도 쉽게 얻을 수 있다. 
더 나아가 학습된 그래프 신경망을 **비슷한 정점 혹은 특성들을 가진 새로운 그래프에도** 적용해볼 수 있다.   
...
#### CNN과 GCN
정점 임베딩에 신경망 구조를 활용하라고하면, 정점들로 만든 adjacency matrix에 CNN을 적용하는 모습을 떠올릴 수 있다.   
  
하지만 우리가 이미지에 CNN을 적용할 때와 달리 그래프에는 **인접한 행이나 열 간의 연관성을 찾기 어렵다.** 
...
더불어 조금의 형태 변화로도 생각보다 큰 성능차이를 불러올 수 있다는 점을 기억하자.
...
이에 따라 도입된 GAT(Graph ATtention network)에서는 **가중치까지 학습시킨다.**  

![GCT](/img/posts/25-4.png){: width="90%" height="90%"}{: .center}   
위 그림과 같이 self-attention 기반 신경망을 구성하여 가중치 $\alpha\_{ij}$ 또한 학습시킬 수 있게 된다.   
  
GCT에서의 self-attention은 transformer의 그것과는 조금 다르다.
...
그전에 먼저 $i$번째 노드에 대한 $j$번째 노드의 중요도(importance) $e\_{ij}$를 구해야한다.   
  
이를 위해 (1) $i$번째 정점과 $j$번째 정점에 $F^{\prime} \times F$인 가중치 $\mathrm{W}$를 각각 곱하여 linear transformation을 한 이후 
...
이 때 모든 정점에 대하여 **가중치 $\mathrm{W}$와 attention mechanism ${a}$는 공유된다.** 따라서 위와 같은 과정을 수행하여 어떤 정점에 대한 이웃 정점의 중요도를 모두 구할 수 있다.   
...
이렇게 완성된 인코딩 벡터를 이용함으로써 정점 분류의 정확도가 기존 모델들에 비해 월등히 향상되었음이 논문에서 제시되었다. 
한편, GAT는 논문에서도 설명이 꽤 쉽게 나와있으므로 보다 자세한 이해를 위해 논문을 한번 읽어보는 것도 좋다.  
